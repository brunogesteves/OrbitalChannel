...
class SchedulePosts
{
    ...
    public function index()
    {
      ...
      $postfaceId=  curl_exec($ch);

        curl_close($ch);
        
        if($$postfaceId){
            setcookie("$postfaceId", $postfaceId . htmlspecialchars(basename($target_file)), time() + (86400), "/");
        }else{
            setcookie("$postfaceId", "erro" . htmlspecialchars(basename($target_file)), time() + (86400), "/");
