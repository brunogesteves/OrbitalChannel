...
for ($x = 0; $x <= sizeof($originalPosts); $x++) {
    ...
    if ($originalPosts[$x]["status"] == "on") {
        if($originalPosts[$x]["section"] === "n1" && sizeof($level1)<=2){
            array_push($level1, $originalPosts[$x]);
        }else if($originalPosts[$x]["section"] === "n2" && sizeof($level2)<=2){
            array_push($level2, $originalPosts[$x]);
        }else if($originalPosts[$x]["section"] === "n3" && sizeof($level3)<=6){
...
for ($x = 0; $x <= sizeof($extposts); $x++) {
        if($extposts[$x]["section"] === "n1" && sizeof($level1)<=2){
            array_push($level1, $extposts[$x]);
        }else if($extposts[$x]["section"] === "n2" && sizeof($level1)<=2){
            array_push($level2, $extposts[$x]);
        }else if($extposts[$x]["section"] === "n3" && sizeof($level3)<=6){
